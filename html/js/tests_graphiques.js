...
{		
	...
	var CanvasManager = function()
	{
		...
		createArcs(t.layer, couleur1, couleur2, couleur3);
		createMotifs(t.layer, {type:1, couleur:couleur1}, {type:2, couleur:couleur2}, {type:3, couleur:couleur1});
...
/*
	...
	AS3 : écran des indices	

	allShapes = new Sprite();
	addChild(allShapes);
	
	var noMotCle:int;
	var n:int;
	var shape:MotifIndice;
	var meta:Meta;
	var itemsAssocies:Array;
	
	var pi:Number = Math.PI;
	var angleStep:Number;
	var radius:Number;
	var degToRad:Number = pi / 180;
	var degs:Number;
	var radians:Number;
	...
	{
		meta = motsClesType1[noMotCle];
		itemsAssocies = _itemsOfQuery.getItemsWithMeta(meta.id)
		
		shape = createShape(1, noMotCle, meta, itemsAssocies.length);
		
		degs = noMotCle * angleStep;
		radians = degs * degToRad;
		
		shape.rotation = 180 - degs;
		shape.x = radius * Math.sin(radians);
		shape.y = radius * Math.cos(radians);
		
		allShapes.addChildAt(shape, 0);
	}
	...
	for (noMotCle=0; noMotCle<n; noMotCle++)
	{
		meta = motsClesType2[noMotCle];
		itemsAssocies = _itemsOfQuery.getItemsWithMeta(meta.id)
		
		shape = createShape(2, noMotCle, meta, itemsAssocies.length);
	...
	for (noMotCle=0; noMotCle<n; noMotCle++)
	{
		meta = motsClesType3[noMotCle];
		itemsAssocies = _itemsOfQuery.getItemsWithMeta(meta.id)
		
		shape = createShape(3, noMotCle, meta, itemsAssocies.length);
